          ...
          <view class="amount-wrapper">
            ...
            <view class="quick-amounts">
              <view
                ...
                :key="amt"
                :class="['amount-tag', { active: formData.amount == amt }]"
...
async function submitOrder() {
  ...
  formRef.value.validate().then(async (res) => {
    ...
    try {
      ...
      setTimeout(() => {
        uni.redirectTo({
          url: `/pages/mutual/payment/payment?orderId=${order.id}&description=${encodeURIComponent(orderData.description)}&helpTime=${encodeURIComponent(orderData.helpTime)}&phone=${encodeURIComponent(orderData.phone)}&campus=${encodeURIComponent(orderData.campus)}&amount=${orderData.amount}`
        })
      }, 1000)
    } catch (e) {
      uni.hideLoading()
