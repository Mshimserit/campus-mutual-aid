...
const platformFee = computed(() => {
  return (orderInfo.value.amount * PLATFORM_FEE_RATE).toFixed(2)
})

const totalAmount = computed(() => {
  return (parseFloat(orderInfo.value.amount) + parseFloat(platformFee.value)).toFixed(2)
})
...
async function handlePayment() {
  ...
  try {
    ...
    await orderService.payOrder(orderId.value)
    
    const order = orderStore.orders.find(o => o.id === orderId.value)
    if (order && order.paid) {
