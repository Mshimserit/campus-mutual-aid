...
const canDeliver = computed(() => {
  return isAcceptor.value && ['IN_PROGRESS', 'ACCEPTED'].includes(order.value?.status)
})
...
function goToPay() {
  uni.navigateTo({
    url: `/pagesSub/mutual/payment/payment?orderId=${order.value.id}&description=${encodeURIComponent(order.value.description)}&helpTime=${encodeURIComponent(order.value.helpTime)}&phone=${encodeURIComponent(order.value.phone)}&campus=${encodeURIComponent(order.value.campus)}&amount=${order.value.amount}`
  })
}
