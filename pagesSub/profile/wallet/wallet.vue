      ...
      <view class="bill-list">
        ...
        <view v-else>
          <view
            ...
            :key="bill.id"
            class="bill-item"
            :class="{ income: bill.type === 'income', expense: bill.type === 'withdraw' }"
          >
            ...
            <view class="bill-amount">
              ...
              <text class="balance-text">余额{{ bill.balance.toFixed(2) }}</text>
            </view>
          </view>
        </view>
      </view>
...
const billFilters = [
  ...
  { label: '支出', value: 'withdraw' },
  { label: '退款', value: 'refund' }
]

const filteredBills = computed(() => {
  if (currentFilter.value === 'all') return walletStore.transactions
  return walletStore.transactions.filter(b => b.type === currentFilter.value)
})

const withdrawRecords = computed(() => {
  return walletStore.transactions.filter(t => t.type === 'withdraw')
})
...
async function loadWalletData() {
  try {
    ...
    // bills.value and withdrawRecords are now from walletStore.transactions
  } catch (e) {
...
function showWithdraw() {
  uni.navigateTo({ url: '/pagesSub/profile/wallet/withdraw' })
}
...
function goToService() {
  uni.navigateTo({ url: '/pagesSub/profile/service/customer-service' })
}
</script>
